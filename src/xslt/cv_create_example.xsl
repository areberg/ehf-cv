...
<xsl:stylesheet xmlns:n1="urn:fdc:difi.no:2017:vefa:structure:Syntax-1"
    ...
    xmlns:cv="urn:fdc:anskaffelser.no:2019:ehf:schema:CV-1"
    version="2.0">
    ...
    <xsl:template match="n1:Syntax/n1:Document">
        ...
        <xsl:element name="{$root}">
            <xsl:namespace name="cv">urn:fdc:anskaffelser.no:2019:ehf:schema:CV-1</xsl:namespace>
    ...
    <xsl:template match="n1:Element[n1:Value/@type='FIXED']">
        <xsl:param name="tag" select="n1:Term"/>
        <xsl:element name="{$tag}">
            <xsl:value-of select="n1:Value[@type='FIXED']"/>
        </xsl:element>
    </xsl:template>
